$( "<div id=\"chartContainer\" style=\"padding-bottom: 300px;\"></div>" ).insertBefore( ".restaurant-items-wrapper:eq(1)" );
...
chrome.runtime.sendMessage({type: 'get_popularity', rest_name: rest_name}, function(response) {
	console.log("Blah4...");
});

// window.onload = function () {
// 	console.log("rufibnisduhifuhdifsuhidu");
...
chrome.runtime.onMessage.addListener(
 function(request, sender) {
	...
	} else if (request.type == "get_popularity") {
		show_popular(request.message);
	}
  });
...
function show_rank(rank_array) {
	...
	var title = $("h1.title").find("span:eq(0)").html()
...
function show_popular(message) {
	var title = $("h1.title").find("span:eq(0)").html();
	if (!message.popular) {
		$("h1.title").find("span:eq(0)").html(title + "&nbsp; <span id=\"blah\" style=\"color: red\">(Not Popular)</span>")	
	}
